...
// 🔥 NUEVO: OBTENER ALERTAS DE LOTES VENCIDOS
$lotesVencidosSQL = "
SELECT l.codigo_barra, p.nombre_producto, l.cantidad, l.fecha_caducidad
FROM lotes l
...
$vencidosCount = mysqli_num_rows($lotesVencidos);

// 🔥 NUEVO: OBTENER ALERTAS DE LOTES PRÓXIMOS A VENCER (7 días)
$lotesProximosSQL = "
SELECT l.codigo_barra, p.nombre_producto, l.cantidad, l.fecha_caducidad
FROM lotes l
INNER JOIN productos p ON l.codigo_barra = p.codigo_barra
...
LIMIT 5
";
$lotesProximos = mysqli_query($conn, $lotesProximosSQL);
$proximosCount = mysqli_num_rows($lotesProximos);

// 🔥 NUEVO: OBTENER PRODUCTOS CON STOCK BAJO
$stockBajoSQL = "
SELECT codigo_barra, nombre_producto, cantidad, stock_minimo
FROM productos
WHERE cantidad <= stock_minimo
ORDER BY cantidad ASC
LIMIT 5
";
$stockBajo = mysqli_query($conn, $stockBajoSQL);
$stockBajoCount = mysqli_num_rows($stockBajo);
...
<head>
    ...
    <!-- CSS DASHBOARD -->
    <link rel="stylesheet" href="../../css/AdminPanel-STYLE/dash.css">
...
<body>
    ...
    <div class="contenido-principal">
        ...
        <div class="dashboard-container">
            ...
            <!-- ===============================
SECCIÓN PRINCIPAL (GRÁFICA + ACTIVIDAD)
================================-->
            ...
            <!-- ===============================
NUEVA DISPOSICIÓN (MÉTRICAS IZQUIERDA + ALERTAS DERECHA)
================================-->
            <div class="seccion-principal">

                <!-- 🔥 MÉTRICAS DE VENTAS (AHORA A LA IZQUIERDA) -->
                <div class="panel">
                    <div class="panel-header">
                ...
                <!-- 🔥 ALERTAS IMPORTANTES (AHORA A LA DERECHA - Recuadro Rojo) -->
                <div class="panel panel-alertas">
                    <div class="panel-header">
                        <h3>⚠️ Alertas Importantes</h3>
                        <span class="badge-alertas">
                            <?= $vencidosCount + $proximosCount + $stockBajoCount ?> alertas
                        </span>
                    </div>
                    <div class="alertas-lista">
                        ...
                        <div class="alerta-tipo-header vencido">
                            <i class="bi bi-x-circle-fill"></i>
                            <span>Lotes Vencidos (' . $vencidosCount . ')</span>
                        </div>';
                            while ($lote = mysqli_fetch_assoc($lotesVencidos)) {
                                echo '
                    <div class="alerta-item">
                        <div class="alerta-icono vencido">
                            <i class="bi bi-box-seam"></i>
                        </div>
                        <div class="alerta-info">
                            <h4>' . htmlspecialchars($lote['nombre_producto']) . '</h4>
                            <p>📦 ' . $lote['cantidad'] . ' unidades | Venció: ' . date('d/m/Y', strtotime($lote['fecha_caducidad'])) . '</p>
                        </div>
                    </div>';
                            }
                            echo '</div>';
                        ...
                        if ($proximosCount > 0) {
                            $totalAlertas += $proximosCount;
                            echo '<div class="alerta-tipo">
                        <div class="alerta-tipo-header proximo">
                            <i class="bi bi-exclamation-triangle-fill"></i>
                            <span>Próximos a Vencer (' . $proximosCount . ')</span>
                        </div>';
                            while ($lote = mysqli_fetch_assoc($lotesProximos)) {
                                echo '
                    <div class="alerta-item">
                        <div class="alerta-icono proximo">
                            <i class="bi bi-box-seam"></i>
                        </div>
                        <div class="alerta-info">
                            <h4>' . htmlspecialchars($lote['nombre_producto']) . '</h4>
                            <p>📦 ' . $lote['cantidad'] . ' unidades | Vence: ' . date('d/m/Y', strtotime($lote['fecha_caducidad'])) . '</p>
                        </div>
                    </div>';
                            }
                            echo '</div>';
                        ...
                        // 🟠 Stock Bajo
                        if ($stockBajoCount > 0) {
                            $totalAlertas += $stockBajoCount;
                            echo '<div class="alerta-tipo">
                        <div class="alerta-tipo-header stock">
                            <i class="bi bi-cart-x-fill"></i>
                            <span>Stock Bajo (' . $stockBajoCount . ')</span>
                        </div>';
                            while ($prod = mysqli_fetch_assoc($stockBajo)) {
                                echo '
                    <div class="alerta-item">
                        <div class="alerta-icono stock">
                            <i class="bi bi-box-seam"></i>
                        </div>
                        <div class="alerta-info">
                            <h4>' . htmlspecialchars($prod['nombre_producto']) . '</h4>
                            <p>📦 ' . $prod['cantidad'] . '/' . $prod['stock_minimo'] . ' unidades mínimas</p>
                        </div>
                    </div>';
                            }
                            echo '</div>';
